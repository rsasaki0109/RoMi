...
The visual simulator shows a semi-humanoid mobile manipulator navigating to a work area, reaching for an object, placing it in a bin, and exposing RoMi-shaped stream counts, runtime stages, policy proposals, and event envelopes. The RoMi Studio mini inspector can switch between live stream freshness, replay graph state, proposed policy actions with observation-window freshness, and dataset report views, with seek controls, runtime graph inspection, and event-envelope inspection for replay debugging.
